...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/30 16:39:40 by cmeston           #+#    #+#             */
/*   Updated: 2022/12/30 16:39:48 by cmeston          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	closewin(t_data *data)
{
	...
	while (i < 2)
	{
		mlx_destroy_image(data->mlx_id, data->mm[i].img);
		i++;
	}
	destroy(data);
	exit(0);
...
void	redo(t_data *data)
{
	...
	if (data->mm[1].img)
		mlx_destroy_image(data->mlx_id, data->mm[1].img);
	draw_2d_map(data->map, data->player, data);
}
